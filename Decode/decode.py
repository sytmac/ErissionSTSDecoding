...
class STSdecode():
    ...
    def __init__(self , s_inputlist):
        ...
        self.__file = ''
        self.__filename = s_inputlist[0]
        print "正在进行解析的文件"
        print"{"
        for i in range(0 , len(s_inputlist)):
            p_binfile = open(s_inputlist[i] , "rb")
            print str(s_inputlist[i])
            self.__file = self.__file+p_binfile.read()
            p_binfile.close()
        print "}"        
        ...
        if(len(self.__file)>0):
            ...
            if(s_timestart[10:12] == "00" and s_timeend[10:12] == "00"):
                ...
                while(i_pos < len(self.__file)):
                    if((binascii.hexlify(s_filestring[i_pos]) == "a2")
                       and(binascii.hexlify(s_filestring[i_pos+1]) == "80")
                       and(binascii.hexlify(s_filestring[i_pos+2]) == "13")):
                        i_pos = i_pos+3
                        #定义一个字典类型的                        
                        d_dictionary = {}                        
                        d_dictionary["ID"] = ""                       
                        ...
                        #在每个OT的字段定义处循环
                        l_ot_columnname = []
                        s_counternamelen = int(binascii.hexlify(s_filestring[i_pos]) , 16)
                        #将字典的key存入列表中
                        ...
                        l_ot_columnname.append(s_filestring[i_pos+1:i_pos+1+s_counternamelen])
                        i_pos = i_pos+s_counternamelen+2#加一的目的是过滤掉无用的字符
                        while(binascii.hexlify(s_filestring[i_pos-1]) != "00"
                            and binascii.hexlify(s_filestring[i_pos]) != "00"
                            and binascii.hexlify(s_filestring[i_pos+1]) != "a3"
                            and binascii.hexlify(s_filestring[i_pos+2]) != "80"):
                            s_counternamelen = int(binascii.hexlify(s_filestring[i_pos]) , 16)
                            l_ot_columnname.append(s_filestring[i_pos+1:i_pos+1+s_counternamelen])
                            i_pos = i_pos+s_counternamelen+2#加一的目的是过滤掉无用的字符
                        ...
                        if(binascii.hexlify(s_filestring[i_pos]) == "30"
                           and binascii.hexlify(s_filestring[i_pos+1]) == "80"
                           and binascii.hexlify(s_filestring[i_pos+2]) == "80"):
                            s_flag_ot = False
                            while(binascii.hexlify(s_filestring[i_pos]) == "30"
                                  and binascii.hexlify(s_filestring[i_pos+1]) == "80"
                                  and binascii.hexlify(s_filestring[i_pos+2]) == "80" 
                                  and s_objlev != "NODEF"):
                                i_pos = i_pos+3
                                s_otmolen = int(binascii.hexlify(s_filestring[i_pos]) , 16)
                                s_otmo = s_filestring[i_pos+1:i_pos+1+s_otmolen]
                                ...
                                if(D_OBJ_LEVEL_DICTIONARY.has_key(s_objtypename)):
                                    ...
                                    #转到判断是否开始取值处
                                    i_pos = i_pos+int(binascii.hexlify(s_filestring[i_pos]) , 16)+1
                                    if(binascii.hexlify(s_filestring[i_pos]) == "a1"
                                       and binascii.hexlify(s_filestring[i_pos+1]) == "80"):
                                        i_pos = i_pos+2
                                        i_data = 6
                                       
                                        while(binascii.hexlify(s_filestring[i_pos])!="00"
                                        or binascii.hexlify(s_filestring[i_pos+1])!="00"
                                        or binascii.hexlify(s_filestring[i_pos+2])!="82"
                                        or binascii.hexlify(s_filestring[i_pos+3])!="01"
                                        or binascii.hexlify(s_filestring[i_pos+4])!="00"
                                        or binascii.hexlify(s_filestring[i_pos+5])!="00"
                                        or binascii.hexlify(s_filestring[i_pos+6])!="00"):
                                            
                                            i_value = 0
                                            
                                            if(binascii.hexlify(s_filestring[i_pos]) == "80"):
                                                i_pos = i_pos+1#游标转移到counter的值长度位置上
                                                s_coutervaluelen = int(binascii.hexlify(s_filestring[i_pos]) , 16)
                                                for i in range(1 , s_coutervaluelen+1):
                                                    i_value = i_value+ int(binascii.hexlify(s_filestring[i_pos+i]) , 16)*pow(16 , 2*(s_coutervaluelen-i))
                                                d_dictionary[l_ot_columnname[i_data]] = str(i_value)
                                                i_pos = i_pos+s_coutervaluelen+1  
                                                i_data = i_data+1
                                            elif(binascii.hexlify(s_filestring[i_pos]) == "82"):
                                                d_dictionary[l_ot_columnname[i_data]] = "0"
                                                i_pos = i_pos+2
                                                i_data = i_data+1
                                            else:
                                                i_pos = i_pos+1 
                                    if (binascii.hexlify(s_filestring[i_pos]) == "00"
                                        and binascii.hexlify(s_filestring[i_pos+1]) == "00"
                                        and binascii.hexlify(s_filestring[i_pos+2]) == "82"
                                        and binascii.hexlify(s_filestring[i_pos+3]) == "01"
                                        and binascii.hexlify(s_filestring[i_pos+4]) == "00"
                                        and binascii.hexlify(s_filestring[i_pos+5])== "00"
                                        and binascii.hexlify(s_filestring[i_pos+6]) == "00"):
                                    ...
                                    s_objlev = "NODEF"
                    i_pos = i_pos+1
    def f_objtype_into_samelevlist(self):
        '''/
        '''
    ...
    #将不同OT表进行合并        
    def f_sameleveltableintoonetable(self):
        #同一种的OT属性进行输出 ， 为了入数据库
                ...
                for j in range(0 , len(D_DICTIONARY_LEVEL_TO_OTLIST[L_LEVELTYPE_LIST[i]][0])):                       
                    ...
                    for k in range(0 , len(D_DICTIONARY_LEVEL_TO_OTLIST[L_LEVELTYPE_LIST[i]])):#k指同一个oT的表的个数                       
                        try:
                            ...
                            print "index error"
                            pass
                    self.f_insertdataintofile(L_LEVELTYPE_LIST[i] , counter_data)                             
    ...
    def f_openfile(self):
        ...
        '''
        #如果不存在就 将列名写进去
        if ((os.path.isfile('../result_table/STSBSC')) == False): 
            p_fp = open('../result_table/STSBSC','a')
            p_fp.write(','.join(D_DICTIONARY_COUNTERCOLUMNNAME_FOR_EACHLEVEL["STSBSC"])+'\n')
            p_fp.close()
        if ((os.path.isfile('../result_table/STSTRA')) == False):
            p_fp = open('../result_table/STSTRA','a')
            p_fp.write(','.join(D_DICTIONARY_COUNTERCOLUMNNAME_FOR_EACHLEVEL["STSTRA"])+'\n')
            p_fp.close()
        if ((os.path.isfile('../result_table/STSCELL')) == False):
            p_fp = open('../result_table/STSCELL','a')
            p_fp.write(','.join(D_DICTIONARY_COUNTERCOLUMNNAME_FOR_EACHLEVEL["STSCELL"])+'\n')
            p_fp.close()
        if ((os.path.isfile('../result_table/STSLAPD')) == False):
            p_fp = open('../result_table/STSLAPD','a')
            p_fp.write(','.join(D_DICTIONARY_COUNTERCOLUMNNAME_FOR_EACHLEVEL["STSLAPD"])+'\n')
            p_fp.close()
        if ((os.path.isfile('../result_table/STSHOEXT')) == False):
            p_fp = open('../result_table/STSHOEXT','a')
            p_fp.write(','.join(D_DICTIONARY_COUNTERCOLUMNNAME_FOR_EACHLEVEL["STSHOEXT"])+'\n')
            p_fp.close()
        if ((os.path.isfile('../result_table/STSMOTS')) == False):
            p_fp = open('../result_table/STSMOTS','a')
            p_fp.write(','.join(D_DICTIONARY_COUNTERCOLUMNNAME_FOR_EACHLEVEL["STSMOTS"])+'\n')
            p_fp.close()
        if ((os.path.isfile('../result_table/STSHOINT')) == False): 
            p_fp = open('../result_table/STSHOINT','a')
            p_fp.write(','.join(D_DICTIONARY_COUNTERCOLUMNNAME_FOR_EACHLEVEL["STSHOINT"])+'\n')
            p_fp.close()
        self.__fp_sts_bsc = open("../result_table/STSBSC","a")
        self.__fp_sts_tra = open("../result_table/STSTRA","a")
        self.__fp_sts_cell = open("../result_table/STSCELL","a")
        self.__fp_sts_lapd = open("../result_table/STSLAPD","a")
        self.__fp_sts_mots = open("../result_table/STSMOTS","a")
        self.__fp_sts_hoint = open("../result_table/STSHOINT","a")
        self.__fp_sts_hoext = open("../result_table/STSHOEXT","a")

    def f_closefile(self):
        '''/
        关闭打开的文件
        '''
    ...
    def f_insertdataintofile(self , s_level , s_counter_data):
        if(s_level == "STSBSC"):
